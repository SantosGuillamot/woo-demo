<?php
/**
 * THIS CODE IS COPIED FROM GUTENBERG TO ALWAYS ENABLE THE FULL CLIENT-SIDE NAVIGATION EXPERIMENT.
 * See: https://github.com/WordPress/gutenberg/blob/trunk/lib/experimental/full-page-client-side-navigation.php
 */

/**
 * Enqueue the interactivity router script.
 */
function woodemo_enqueue_interactivity_router() {
...
add_action( 'wp_enqueue_scripts', 'woodemo_enqueue_interactivity_router' );
 ...
 */
function woodemo_add_enhanced_pagination_to_query_block( $parsed_block ) {
...
add_filter( 'render_block_data', 'woodemo_add_enhanced_pagination_to_query_block' );
 ...
 */
function woodemo_add_client_side_navigation_directives( $response_body ) {
...
add_filter( 'gutenberg_template_output_buffer', 'woodemo_add_client_side_navigation_directives' );
 ...
 */
function woodemo_buffer_template_output( string $passthrough ): string {
	ob_start(
...
add_filter( 'template_include', 'woodemo_buffer_template_output', PHP_INT_MAX );
